...
> Wake word detection uses **openWakeWord** (open-source, no account needed).
> The chosen model downloads automatically from HuggingFace on first boot.
...
2. Clone the repo and run the USB preparation script:
   ...
   ```
   When prompted, enter your two API keys. The script encrypts them and writes all necessary files.

3. Commit `keys.enc` to your GitHub repo (it is AES-256 encrypted — safe to commit):
   ```bash
   git add keys.enc
...
| No wake word response | `logs/freezerbot.log` — check openWakeWord model downloaded correctly |
